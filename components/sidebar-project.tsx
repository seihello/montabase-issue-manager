...
export default function SidebarProject({ project, isSelected }: Props) {
  ...
  return (
    <div
      className={`relative inline-flex h-8 w-full cursor-pointer items-center justify-start whitespace-nowrap rounded-md px-2 py-2 text-sm font-medium ring-offset-background transition-colors hover:text-accent-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 ${isSelected ? "bg-blue-100" : "hover:bg-accent"}`}
      onClick={() => router.push(`/project/${project.id}`)}
    >
      <span className="truncate w-40">{project.title}</span>
      <ProjectMenu
